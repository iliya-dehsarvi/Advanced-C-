...
#include <algorithm>
/*  ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
g++ --std=c++11 STORE.cpp -o STORE && ./STORE
~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~  */
class BARCODES {
...
public:
    ...
    std::vector<ORDER_INFO> ORDERS() { return _ORDERS; }
};
/*~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~*/
struct HASH_NODE {
    std::string CART_ID;
...
public:
    void PUSH_BACK(ORDER_INFO ORDER) {
    ...
    std::vector<HASH_NODE> TABLE() { return _TABLE; }
};
/*~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~*/
class STORE {
    XML_PROCESSOR _STORE_DATABASE;
    CUSTOMER_ORDERS _STORE_ORDERS;
    PRODUCT_DATABASE _DATABASE;
    std::vector<ORDER_INFO> _ORDERS;
    HASH _HASHED_ORDERS;
    void _SYNC() {
        for (auto CUSTOMER: _ORDERS) { _HASHED_ORDERS.PUSH_BACK(CUSTOMER); }
    }
public:
    STORE() {
        _DATABASE = _STORE_DATABASE.DB();
        _ORDERS = _STORE_ORDERS.ORDERS();
        _SYNC();
        DISPLAY_ORDERS();
    }
    void DISPLAY_ORDERS() {
        double TOTAL;
        for (auto DATA: _HASHED_ORDERS.TABLE()) {
            std::cout << "\n" << DATA.CART_ID << "\n- - - - - - - - - - - - - - - --\n";
            TOTAL=0;
            for (auto _DATA: DATA.ITEMS) {
                TOTAL+=_DATA._PRICE;
...
public:
    STORE() {
        ...
        _ORDERS = _STORE_ORDERS.ORDERS();
        for (auto CUSTOMER: _ORDERS) { _HASHED_ORDERS.PUSH_BACK(CUSTOMER); }
        THREADING LANES(_HASHED_ORDERS.TABLE());
    }
};
/*~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~*/
